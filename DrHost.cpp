  ...
  : DrRetro(parent), m_config(config)
{
  ...
  connect(m_core, &QRetro::frameBegin, this, [this]() {
    ...
    for (unsigned r = 0; r < m_config.scene_board_range_count; r++) {
      if (val >= m_config.scene_board_ranges[r].min &&
          val <= m_config.scene_board_ranges[r].max) {
    ...
    if (val == m_config.scene_miniexplain) {
      ...
      if (m_config.minigame_type_addr &&
          prevMinigameType < m_config.minigame_type_to_dr_size)
      ...
      if (mgType == DR_MINIGAME_1V3) {
        for (unsigned i = 0; i < 4; i++)
          if (playerValid[i])
            players[i].team_type = (players[i].team_id == 0)
