...
static const size_t MT_CHARACTER_ADDR[4] = {
  0x80065214, 0x80065218, 0x8006521C, 0x80065220,
};

static const size_t MT_COLOR_ADDR[4] = {
  0x80065224, 0x80065228, 0x8006522C, 0x80065230,
};
...
static const size_t MT_SLOT_ADDR[4] = {
  0x80065237, 0x80065236, 0x80065235, 0x80065234,
};
...
static const size_t MT_DIFFICULTY_ADDR[4] = {
  0x8006523F, 0x8006523E, 0x8006523D, 0x8006523C,
};
...
static const mt_character_t k_drToChar[DR_CHARACTER_SIZE] = {
  ...
  [DR_CHARACTER_KOOPA_KID]   = { 0x0F, 0x01 },
...
MarioTennis::MarioTennis(QObject *parent) : DrGuest(parent)
{
  m_core    = new QRetro();
  m_ownCore = true;
...
void MarioTennis::run()
{
  ...
  for (unsigned team = 0; team < 2; team++) {
    uint8_t setsWon = 0;
    if (readu8(&setsWon, MT_SETS_WON_ADDR[team]) != DR_OK || setsWon < 1)
      continue;
...
void MarioTennis::doSetMinigame(const dr_mp_minigame_t *minigame)
{
  m_winners         = 0;
  m_finishCountdown = 0;
  m_core->unserializeFromFile(dr_state_directory() + "/mariotennis.state.zip");
  ...
  log(DR_LOG_INFO, qPrintable(QString("starting %1: court 0x%2")
    .arg(minigame->name)
    .arg(court, 2, 16, QChar('0'))));
  for (unsigned i = 0; i < 4; i++) {
...
dr_error MarioTennis::doSetPlayerDifficulty(unsigned index, dr_difficulty difficulty)
{
  ...
  case DR_DIFFICULTY_EASY:      val = 0x00; break;
  case DR_DIFFICULTY_NORMAL:    val = 0x01; break;
  case DR_DIFFICULTY_HARD:      val = 0x02; break;
  case DR_DIFFICULTY_VERY_HARD: val = 0x03; break;
  default:                      val = 0x01; break;
  }
  return writeu8(val, MT_DIFFICULTY_ADDR[index]);
}
...
void MarioTennis::applyTeams()
{
  ...
  if (count[0] == 2 && count[1] == 2) {
    slotPlayer[0] = team[0][0]; slotPlayer[1] = team[1][0];
    slotPlayer[2] = team[0][1]; slotPlayer[3] = team[1][1];
    slotCount = 4;
  ...
  for (unsigned s = 0; s < slotCount; s++) {
    ...
    writeu8(static_cast<uint8_t>(pi), MT_SLOT_ADDR[s]);
    ...
    writeu8(ctrl, MT_CONTROL_ADDR[s]);
  }
}

dr_error MarioTennis::doSetPlayerTeam(unsigned index, dr_team_color color, dr_team_type type, unsigned team_id)
{
